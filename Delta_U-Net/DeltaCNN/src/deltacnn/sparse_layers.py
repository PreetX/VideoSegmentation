        ...
        if type(input) == Tensor:
            ...
            if self.ceil_mode:
                if len(self.padding) == 2:
                    out_h = ceil((x.shape[2] + 2 * self.padding[0] - self.dilation[0] * (self.kernel_size[0]-1) - 1) // self.stride[0] + 1)
                    out_w = ceil((x.shape[3] + 2 * self.padding[1] - self.dilation[1] * (self.kernel_size[1]-1) - 1) // self.stride[1] + 1)
                elif len(self.padding) == 4:
                    out_h = ceil((x.shape[2] + self.padding[0] + self.padding[1] - self.dilation[0] * (self.kernel_size[0]-1) - 1) // self.stride[0] + 1)
                    out_w = ceil((x.shape[3] + self.padding[2] + self.padding[3] - self.dilation[1] * (self.kernel_size[1]-1) - 1) // self.stride[1] + 1)
